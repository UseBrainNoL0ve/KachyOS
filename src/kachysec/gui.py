...
def launch_gui() -> int:
    ...
    workflow_help = QLabel(
        "Scope-aware operation planner. Build a reviewable security workflow without executing commands."
    )
    workflow_help.setWordWrap(True)
    workflow_layout.addWidget(workflow_help)
    workflow_row = QHBoxLayout()
    workflow_select = QComboBox()
    workflow_target = QLineEdit("localhost")
    workflow_target.setPlaceholderText("Target / lab scope")
    workflow_auth = QLineEdit("local lab / operator controlled")
    workflow_auth.setPlaceholderText("Authorization statement")
    workflow_tool = QLineEdit()
    workflow_tool.setPlaceholderText("Selected tool (optional)")
    workflow_build = QPushButton("▶ BUILD OPERATION PLAN")
    workflow_row.addWidget(workflow_select, 2)
    workflow_row.addWidget(workflow_target, 1)
    workflow_row.addWidget(workflow_auth, 1)
    workflow_row.addWidget(workflow_tool, 1)
    workflow_row.addWidget(workflow_build)
    workflow_layout.addLayout(workflow_row)
    workflow_output = QTextEdit()
    workflow_output.setReadOnly(True)
    workflow_output.setObjectName("Panel")
    workflow_layout.addWidget(workflow_output, 1)
    tabs.addTab(workflow_page, "⚔  OPERATIONS")
    ...
    workflow_build.clicked.connect(build_operation_plan)
