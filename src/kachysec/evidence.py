...
def diff_baselines(before: dict[str, Any], after: dict[str, Any]) -> BaselineDiff:
    old = _flatten(before)
    new = _flatten(after)
    added = tuple(sorted(key for key in new.keys() - old.keys()))
    removed = tuple(sorted(key for key in old.keys() - new.keys()))
...
def render_diff(diff: BaselineDiff, *, before: Path, after: Path) -> str:
    lines = ["# KachySec Baseline Diff", "", f"- Before: {before}", f"- After: {after}", "", f"Added fields: **{len(diff.added)}**", f"Removed fields: **{len(diff.removed)}**", f"Changed fields: **{len(diff.changed)}**", ""]
    for title, values in (("Added", diff.added), ("Removed", diff.removed), ("Changed", diff.changed)):
        ...
        lines.append("")
    lines.append("This comparison is structural; it does not by itself establish whether a change is secure or malicious.")
    return "\n".join(lines)
